...
const EditPredictionDialog: React.FC<EditPredictionDialogProps> = ({ open, onOpenChange, prediction, onEdit }) => {
  ...
  useEffect(() => {
    if (prediction) {
      ...
      form.setValue('confidence', prediction.confidence);
  ...
  const onSubmit = (data: PredictionForm) => {
    ...
    onEdit(prediction.id, formattedData);
    onOpenChange(false);
    form.reset();
    setNumberInputs(['', '', '']);
  };
...
export default EditPredictionDialog;
